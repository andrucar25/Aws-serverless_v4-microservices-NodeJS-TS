...
export const cleanupCategories = async () => {
  try {
    const tableName = process.env.DYNAMO_TABLE;
    const snsTopicArn = process.env.SNS_TOPIC_ARN;

    //calculate timestamp for one hour ago to filter outdated categories
    ...
    //initialize counter to track number of deleted categories
    let deletedCount = 0;
